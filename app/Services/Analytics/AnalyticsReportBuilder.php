...
class AnalyticsReportBuilder
{
    ...
    /**
     * Runs $compute through the same-day cache; on success also refreshes the
     * long-lived "last known good" copy. If the live query throws (cold and
     * timing out, BigQuery down, …) it falls back to that copy tagged
     * `status: 'stale'` — the page shows yesterday's numbers with an honest
     * "as of <date>" instead of a spinner or a red "failed" badge. Only a
     * genuine first-ever failure (no stale copy) surfaces as `failed`.
     *
     * @param  callable(): array  $compute  returns the full status-tagged report array
     */
    private function withStale(string $key, callable $compute, bool $forceRefresh): array
    {
        try {
            $result = $this->cache->remember($key, $compute, $forceRefresh);
            $this->cache->putStale($key, $result);
            ...
            if (is_array($stale)) {
                return [...$stale, 'status' => 'stale', 'error' => $e->getMessage()];
            }
    ...
    /** Breakdown variant of withStale(): returns the rows, the stale rows, or null. */
    private function breakdownWithStale(string $key, callable $compute, bool $forceRefresh): ?array
    {
        try {
            $result = $this->cache->remember($key, $compute, $forceRefresh);
            $this->cache->putStale($key, $result);
    ...
    public function gscReport(
        ...
        return $this->withStale($key, function () use ($gsc, $domain, $dateFrom, $dateTo, $compareFrom, $compareTo) {
            $hasComparison = $compareFrom !== null && $compareTo !== null;
    ...
    public function ahrefsReport(
        ...
        return $this->withStale($key, function () use ($ahrefs, $domain, $dateFrom, $dateTo, $compareFrom, $compareTo) {
            $hasComparison = $compareFrom !== null && $compareTo !== null;
    ...
    public function websiteComparison(TrafficDashboardQuery $ga4, GscReportQuery $gsc, array $registry, Carbon $dateFrom, Carbon $dateTo, bool $forceRefresh = false): array
    {
        ...
        $ga4Rows = $this->breakdownWithStale($ga4Key, fn () => $ga4->summaryByWebsite($domains, $dateFrom, $dateTo), $forceRefresh);
        $gscRows = $this->breakdownWithStale($gscKey, fn () => $gsc->summaryByWebsite($domains, $dateFrom, $dateTo), $forceRefresh);

        $ga4Summary = ['rows' => $ga4Rows ?? [], 'status' => $ga4Rows === null ? 'failed' : ($ga4Rows === [] ? 'missing' : 'ok'), 'error' => null];
        $gscSummary = ['rows' => $gscRows ?? [], 'status' => $gscRows === null ? 'failed' : ($gscRows === [] ? 'missing' : 'ok'), 'error' => null];
