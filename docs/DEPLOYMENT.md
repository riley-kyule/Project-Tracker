...
- The `app` build target installs `docker-cli`/`docker-cli-compose` and adds `www-data` to a `docker` group. That group's GID must match the host socket's actual group (`stat -c '%g' /var/run/docker.sock`) — a mismatch doesn't fail loudly, it just means the restart step 403s against the socket every time, silently leaving `app`/`scheduler` on stale opcache after every "successful" deploy. `docker-compose.yml` pins this via `build.args.DOCKER_GID` on the `app`/`queue`/`scheduler` services (currently `988`, this host's actual socket GID — re-check with the `stat` command above and update all three if the host ever changes) rather than relying on someone remembering to pass `--build-arg` by hand.
